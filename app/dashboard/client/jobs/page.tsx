...
export default function ClientJobsPage() {
	...
	const handleDeleteRequest = (jobId: string, title: string) => {
		setDeleteCandidate({ id: jobId, title });
	};

	const confirmDelete = async () => {
		if (!deleteCandidate) return;
		const { id: jobId } = deleteCandidate;
	...
	return (
		<div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
								...
								<div className="flex items-start justify-between gap-4">
									...
									<div className="text-right shrink-0 space-y-2">
										...
										{job.status === "open" && (
											<button
												type="button"
												onClick={() => handleDeleteRequest(job.id, job.title)}
			...
			<ActionModal
				...
				description={
					deleteCandidate
						? `"${deleteCandidate.title}" will be removed permanently.`
						: undefined
				}
				confirmLabel="Delete job"
				cancelLabel="Keep job"
				isProcessing={Boolean(deletingJobId)}
			/>
